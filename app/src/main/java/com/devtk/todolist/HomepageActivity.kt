    ...
    val simpleCallback = object : SimpleCallback(0, ItemTouchHelper.LEFT or ItemTouchHelper.RIGHT) {
        ...
        override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
            ...
            if(direction == ItemTouchHelper.LEFT){
                ...
                database.child("Users").child(firebaseAuthCUser).child("Notes").addListenerForSingleValueEvent(object : ValueEventListener {
                        ...
                        snapshot.children.forEach {
                            ...
                            if (values != null) {
                                if (values.task_name == noteName.toString()) {
                                    ...
                                    database.child("Users").child(firebaseAuthCUser).child("Notes").child(key.toString()).removeValue()
                                    Snackbar.make(findViewById(R.id.all_tasks_recycler), noteName, Snackbar.LENGTH_LONG).setAction("Undo", View.OnClickListener {
                                        database.child("Users").child(firebaseAuthCUser).child("Notes").child(deletedData?.key.toString()).setValue(deletedData)
                                        if (deletedData != null) {
                                            adapter.add(position, tasks_viewholder(adapter, deletedData))
                                        }
                                    }).show()
                                }
                            }
                            ...
                            if (values != null) {
                                if (values.task_name == noteName.toString()) {
                                    ...
                                    if (values.status == "INCOMPLETED") {
                                        database.child("Users").child(firebaseAuthCUser).child("Notes").child(key.toString()).child("status").setValue("COMPLETED")
                                        adapter.notifyDataSetChanged()
                                        task_name_tv.paintFlags = task_name_tv.paintFlags or Paint.STRIKE_THRU_TEXT_FLAG
                                    } else if (values.status == "COMPLETED") {
                                        database.child("Users").child(firebaseAuthCUser).child("Notes").child(key.toString()).child("status").setValue("INCOMPLETED")
                                        adapter.notifyDataSetChanged()
        ...
        override fun onChildDraw(c: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float, dY: Float, actionState: Int, isCurrentlyActive: Boolean) {
            RecyclerViewSwipeDecorator.Builder(c, recyclerView, viewHolder, dX, dY, actionState, isCurrentlyActive)
                    .addSwipeLeftBackgroundColor(ContextCompat.getColor(this@HomepageActivity,R.color.red_light))
                    .addSwipeLeftActionIcon(R.drawable.ic_delete)
                    .addSwipeLeftLabel("Delete")
                    .addSwipeRightBackgroundColor(ContextCompat.getColor(this@HomepageActivity,R.color.green))
